...
## ------------------------------------------------------------------------
names(train)<-gsub("requester","req",names(train))
names(test) <-gsub("requester","req",names(test))

justInCase<- as.data.frame(rbind(train[, c(2:4,16:17)],
                                  test[, c(2:4,16:17)]))

train <- as.data.frame(cbind(train[,-c(3:4,16:17)], TextTrain),
                       stringsAsFactors=F)

test <- as.data.frame(cbind(test[,c(3:4,16:17)],  TextTest),
                      stringsAsFactors=F)
